...
func initialize() *cli.App {
	...
	app.Flags = []cli.Flag{
		...
		cli.StringFlag{
			Name:        "git-alias, a",
			Value:       "git",
			Usage:       "alias for git command. Default is git",
			Destination: &git,
		},
	}
	app.Action = func(c *cli.Context) error {
		if c.NArg() == 0 {
			log.Fatal("arguments cannot be empty. See git-delete-branches --help.")
		}
		...
		if len(branches) > 0 {
			...
			log.Fatalf("no branches found with %v", strings.Join(include, ", "))
		}
